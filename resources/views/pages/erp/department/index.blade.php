...
<style>
    ...
    /* Table Container */
    .mirsaige-department-table-wrapper {
        width: 100%;
        overflow-x: auto;
    ...
    .mirsaige-department-table-container {
        background: var(--mirsaige-dark-blue);
        border-radius: 8px;
    ...
    /* Table Styles */
    .mirsaige-department-table {
        width: 100%;
        border-collapse: collapse;
    }

    .mirsaige-department-table thead {
        background: var(--mirsaige-darker-blue);
    }

    .mirsaige-department-table th {
        color: var(--mirsaige-accent);
    ...
    .mirsaige-department-table tr:last-child td {
        border-bottom: none;
    }

    .mirsaige-department-table tr:hover td {
        background: rgba(255, 178, 62, 0.05);
        color: var(--mirsaige-white);
    }

    /* Action Buttons */
    .mirsaige-department-actions {
        display: flex;
        gap: var(--mirsaige-space-xs);
        flex-wrap: wrap;
    ...
    .mirsaige-department-action-btn {
        padding: var(--mirsaige-space-3xs) var(--mirsaige-space-xs);
        border-radius: 4px;
        font-size: 0.8rem;
        font-weight: 500;
    ...
    .mirsaige-department-action-btn.view {
        background: var(--mirsaige-primary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-department-action-btn.edit {
        background: var(--mirsaige-secondary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-department-action-btn.delete {
        background: #dc3545;
        color: var(--mirsaige-white);
    }

    .mirsaige-department-action-btn:hover {
        opacity: 0.9;
        transform: translateY(-1px);
    }

    /* Action text visibility - replaces JavaScript functionality */
    .mirsaige-department-action-btn .action-text {
        display: inline;
        color: var(--mirsaige-white);
    }
    ...
    /* Scrollbar Styling */
    .mirsaige-department-table-wrapper::-webkit-scrollbar {
        height: 8px;
    }

    .mirsaige-department-table-wrapper::-webkit-scrollbar-track {
        background: var(--mirsaige-dark-blue);
        border-radius: 4px;
    }

    .mirsaige-department-table-wrapper::-webkit-scrollbar-thumb {
        background: var(--mirsaige-accent);
        border-radius: 4px;
    }

    .mirsaige-department-table-wrapper::-webkit-scrollbar-thumb:hover {
    ...
    @media (max-width: 767px) {
        ...
        .mirsaige-department-table tbody {
            display: block;
            width: 100%;
        }
        
        .mirsaige-department-table tr {
            display: block;
            margin-bottom: var(--mirsaige-space-md);
        ...
        .mirsaige-department-table td:last-child {
            border-bottom: none;
        }
        
        .mirsaige-department-table td::before {
...
@section('page')
<div class="mirsaige-department-container">
    <div class="mirsaige-department-header">
        <div>
            <h1 class="mirsaige-app-breadcrumbs-title"> Departments List</h1>
            <div class="mirsaige-app-breadcrumbs">
                ...
                <div class="mirsaige-app-breadcrumb">
                    <a href="{{ route('departments.index') }}" class="active">Manage Department</a>
                </div>
                @endif
            </div>
        </div>
        ...
        @if (in_array($user_role_id, [1, 2]))
        <a href="{{ route('departments.create') }}" class="mirsaige-app-breadcrumbs-btn">
            <i class="fa-solid fa-square-plus"></i> <span class="action-text">Create</span>
        </a>
        @endif
    </div>

    <div class="mirsaige-department-table-wrapper">
        <div class="mirsaige-department-table-container">
            <table class="mirsaige-department-table">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if(in_array($user_role_id, [1, 2]))
                        <td data-label="Created By">{{ $department->creator->name ?? 'Unknown' }}</td>
                        <td data-label="Updated By">{{ $department->updater->name ?? 'Unknown' }}</td>
                        @endif
                        <td data-label="Actions">
                            <div class="mirsaige-department-actions">
                                @if (in_array($user_role_id, [1, 2,3]))
                                <a href="{{ route('departments.show', $department->id) }}" class="mirsaige-department-action-btn view">
                                    <i class="fa-solid fa-eye"></i> <span class="action-text">View</span>
                                </a>
                                <a href="{{ route('departments.edit', $department->id) }}" class="mirsaige-department-action-btn edit">
                                ...
                                <form action="{{ route('departments.destroy', $department->id) }}" method="post" style="display: inline;">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="mirsaige-department-action-btn delete">
