...
<style>
    ...
    .mirsaige-price-calculation-row {
        display: flex;
        justify-content: space-between;
    ...
    .mirsaige-price-calculation-row:last-child {
        margin-bottom: 0;
        font-weight: 600;
        color: var(--mirsaige-accent);
    }
...
<div class="mirsaige-product-container">
    <div class="mirsaige-product-header">
        <div class="mirsaige-app-breadcrumbs">
            ...
            <div class="mirsaige-app-breadcrumb">
                <a href="{{ route('products.create') }}" class="active">Create Product</a>
            </div>
        </div>

        <a href="{{ route('products.index') }}" class="mirsaige-app-breadcrumbs-btn">
            <i class="fa-solid fa-list-check"></i> Product List
        </a>
    </div>
            ...
            <div class="mirsaige-product-form-wrapper">
                ...
                <div class="mirsaige-form-half-section">
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="text" class="mirsaige-form-control" name="name" id="name" placeholder="Enter product name" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="number" step="0.01" class="mirsaige-form-control" name="regular_price" id="regular_price" placeholder="Enter regular price" value="{{ old('regular_price') }}" required>
                        @error('regular_price')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="number" step="0.01" class="mirsaige-form-control" name="offer_price" id="offer_price" placeholder="Enter offer price" value="{{ old('offer_price') }}">
                        @error('offer_price')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="number" step="0.01" class="mirsaige-form-control" name="offer_discount" id="offer_discount" placeholder="Enter discount percentage" value="{{ old('offer_discount') }}">
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="number" step="0.1" min="0" max="5" class="mirsaige-form-control" name="star" id="star" placeholder="Enter star rating (0-5)" value="{{ old('star') }}">
                        @error('star')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="text" class="mirsaige-form-control" name="barcode" id="barcode" placeholder="Enter barcode" value="{{ old('barcode') }}">
                        @error('barcode')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                ...
                <!-- Right Section - Additional Product Info -->
                <div class="mirsaige-form-half-section">
                    ...
                    <div class="mirsaige-form-group">
                        ...
                        <input type="number" step="0.01" class="mirsaige-form-control" name="weight" id="weight" placeholder="Enter product weight" value="{{ old('weight') }}">
                        @error('weight')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="mirsaige-form-group full-width">
                        ...
                        <textarea class="mirsaige-form-control mirsaige-form-textarea" name="description" id="description" placeholder="Enter product description">{{ old('description') }}</textarea>
                        @error('description')
                        <small class="mirsaige-form-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="mirsaige-price-calculation" id="priceCalculation" style="display: none;">
                        <div class="mirsaige-price-calculation-row">
                            <span>Regular Price:</span>
                            <span id="regularPriceDisplay">$0.00</span>
                        </div>
                        <div class="mirsaige-price-calculation-row">
                            <span>Discount:</span>
                            <span id="discountDisplay">$0.00 (0%)</span>
                        </div>
                        <div class="mirsaige-price-calculation-row">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const discountInput = document.getElementById('offer_discount');
        ...
        function calculatePrices() {
            ...
            // Calculate discount if offer price is provided but discount is not
            if (offerPrice > 0 && discount === 0 && regularPrice > 0) {
                discount = ((regularPrice - offerPrice) / regularPrice) * 100;
                discountInput.value = discount.toFixed(2);
            }
            
            // Calculate offer price if discount is provided but offer price is not
            if (discount > 0 && offerPrice === 0 && regularPrice > 0) {
                offerPrice = regularPrice - (regularPrice * discount / 100);
                offerPriceInput.value = offerPrice.toFixed(2);
            }
            
            // Update display
            regularPriceDisplay.textContent = '$' + regularPrice.toFixed(2);
            
            if (discount > 0) {
                const discountAmount = regularPrice * discount / 100;
                discountDisplay.textContent = '$' + discountAmount.toFixed(2) + ' (' + discount.toFixed(2) + '%)';
                finalPriceDisplay.textContent = '$' + (regularPrice - discountAmount).toFixed(2);
                priceCalculation.style.display = 'block';
            } else if (offerPrice > 0) {
                const calculatedDiscount = ((regularPrice - offerPrice) / regularPrice) * 100;
                discountDisplay.textContent = '$' + (regularPrice - offerPrice).toFixed(2) + ' (' + calculatedDiscount.toFixed(2) + '%)';
                finalPriceDisplay.textContent = '$' + offerPrice.toFixed(2);
                priceCalculation.style.display = 'block';
            } else {
        ...
        regularPriceInput.addEventListener('input', calculatePrices);
        offerPriceInput.addEventListener('input', calculatePrices);
        ...
        form.addEventListener('submit', function(e) {
            // Validate that regular price is provided
            if (!regularPriceInput.value || parseFloat(regularPriceInput.value) <= 0) {
                e.preventDefault();
                showAlert('Error', 'Regular price must be greater than 0', 'error');
