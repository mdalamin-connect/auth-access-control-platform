...
<style>
    ...
    .mirsaige-status-table-container {
        background: var(--mirsaige-dark-blue);
        border-radius: 8px;
    ...
    /* Table Styles */
    .mirsaige-status-table {
        width: 100%;
        border-collapse: collapse;
    }

    .mirsaige-status-table thead {
        background: var(--mirsaige-darker-blue);
    }

    .mirsaige-status-table th {
        color: var(--mirsaige-accent);
    ...
    .mirsaige-status-table tr:last-child td {
        border-bottom: none;
    }

    .mirsaige-status-table tr:hover td {
        background: rgba(255, 178, 62, 0.05);
        color: var(--mirsaige-white);
    }

    /* Action Buttons */
    .mirsaige-status-actions {
        display: flex;
        gap: var(--mirsaige-space-xs);
        flex-wrap: nowrap;
    }

    .mirsaige-status-action-btn {
        padding: var(--mirsaige-space-3xs) var(--mirsaige-space-xs);
        border-radius: 4px;
        font-size: 0.8rem;
        font-weight: 500;
    ...
    .mirsaige-status-action-btn.view {
        background: var(--mirsaige-primary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-status-action-btn.edit {
        background: var(--mirsaige-secondary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-status-action-btn.delete {
        background:rgb(220, 51, 68);
        color:rgb(137, 3, 3);
    }

    .mirsaige-status-action-btn:hover {
        opacity: 0.9;
        transform: translateY(-1px);
    }

    /* Action text visibility - replaces JavaScript functionality */
    .mirsaige-status-action-btn .action-text {
        display: inline;
        color: var(--mirsaige-white);
    }
    ...
    /* Scrollbar Styling */
    .mirsaige-status-table-wrapper::-webkit-scrollbar {
        height: 8px;
    }

    .mirsaige-status-table-wrapper::-webkit-scrollbar-track {
        background: var(--mirsaige-dark-blue);
        border-radius: 4px;
    }

    .mirsaige-status-table-wrapper::-webkit-scrollbar-thumb {
        background: var(--mirsaige-accent);
        border-radius: 4px;
    }

    .mirsaige-status-table-wrapper::-webkit-scrollbar-thumb:hover {
    ...
    @media (max-width: 767px) {
        ...
        .mirsaige-status-table tbody {
            display: block;
            width: 100%;
        }
        
        .mirsaige-status-table tr {
            display: block;
            margin-bottom: var(--mirsaige-space-md);
        ...
        .mirsaige-status-table td:last-child {
            border-bottom: none;
        }
        
        .mirsaige-status-table td::before {
...
@section('page')
<div class="mirsaige-status-container">
    <div class="mirsaige-status-header">
        <div>
            <h1 class="mirsaige-app-breadcrumbs-title">Status List</h1>
            <div class="mirsaige-app-breadcrumbs">
                ...
                <div class="mirsaige-app-breadcrumb">
                    <a href="{{ route('status.index') }}" class="active">Manage Status</a>
                </div>
                @endif
            </div>
        </div>
        ...
        <a href="{{ route('status.create') }}" class="mirsaige-app-breadcrumbs-btn">
            <i class="fa-solid fa-square-plus"></i> <span class="action-text">Create</span>
        </a>
        @endif
    </div>

    <div class="mirsaige-status-table-wrapper">
        <div class="mirsaige-status-table-container">
            <table class="mirsaige-status-table">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td data-label="Description">{{ $st->descriptions }}</td>
                        @if(in_array($user_role_id, [1, 2]))
                        <td data-label="Created By">{{ $st->creator->name ?? 'Unknown' }}</td>
                        <td data-label="Updated By">{{ $st->updater->name ?? 'Unknown' }}</td>
                        @endif
                        <td data-label="Actions">
                            <div class="mirsaige-status-actions">
                                @if (in_array($user_role_id, [1, 2,3]))
                                <a href="{{ route('status.show', $st->id) }}" class="mirsaige-status-action-btn view">
                                    <i class="fa-solid fa-eye"></i> <span class="action-text">View</span>
                                </a>
                                <a href="{{ route('status.edit', $st->id) }}" class="mirsaige-status-action-btn edit">
                                ...
                                <form action="{{ route('status.destroy', $st->id) }}" method="post" style="display: inline;">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="mirsaige-status-action-btn delete">
...
<script>
    ...
    const deleteButtons = document.querySelectorAll('.mirsaige-status-action-btn.delete');
