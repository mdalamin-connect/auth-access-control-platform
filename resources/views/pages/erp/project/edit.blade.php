...
<div class="mirsaige-project-container">
    <div class="mirsaige-project-header">
        <div>
            <div class="mirsaige-app-breadcrumbs">
                ...
                <div class="mirsaige-app-breadcrumb">
                    <a href="#" class="active">Edit Project</a>
                </div>
            </div>
            @if(in_array($user_role_id, [1, 2]))
            <small class="mirsaige-last-updated">
                Last updated by: {{ $project->updater->name ?? 'System' }} on {{ $project->updated_at->format('M d, Y h:i A') }}
            </small>
            @endif
        </div>
    ...
    <div class="mirsaige-project-form-container">
        <form action="{{ route('projects.update', $project) }}" method="post" enctype="multipart/form-data" class="mirsaige-project-form" id="projectForm">
            @csrf
            @method('PUT')
            ...
            <div class="mirsaige-form-group mirsaige-image-upload-container">
                <div class="mirsaige-image-preview" id="imagePreview">
                    @if($project->photo)
                        <img src="{{ asset('img/projects' . $project->photo) }}" alt="Project Image" class="img-fluid">
                    @else
            ...
            <div class="mirsaige-form-group">
                ...
                <input type="text" class="mirsaige-form-control" name="name" id="name" 
                       placeholder="Enter project name" required value="{{ old('name', $project->name) }}">
                @error('name')
                    <span class="mirsaige-error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mirsaige-form-group">
                ...
                <input type="text" class="mirsaige-form-control" name="locations" id="locations" 
                       placeholder="Enter project location(s)" value="{{ old('locations', $project->locations) }}">
                @error('locations')
                    <span class="mirsaige-error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mirsaige-form-group full-width">
                ...
                <textarea class="mirsaige-form-control mirsaige-form-textarea rich-text-editor" 
                        name="descriptions" 
                        id="richTextEditor" 
                        placeholder="Enter project description">{{ old('descriptions', $project->descriptions) }}</textarea>
                @error('descriptions')
                    <span class="mirsaige-error-message">{{ $message }}</span>
                @enderror
            </div>
...
<script>
    $(document).ready(function() {
        ...
        document.getElementById('resetFormBtn').addEventListener('click', function() {
            // Reset the image preview to original
            @if($project->photo)
                imagePreview.innerHTML = `<img src="{{ asset('storage/' . $project->photo) }}" alt="Project Image" class="img-fluid">`;
            @else
            ...
            // Reset Summernote to original content
            $('.rich-text-editor').summernote('code', `{!! old('descriptions', $project->descriptions) !!}`);
        });
