...
<style>
    ...
    /* Table Container */
    .mirsaige-supplier-table-wrapper {
        width: 100%;
        overflow-x: auto;
    ...
    .mirsaige-supplier-table-container {
        background: var(--mirsaige-dark-blue);
        border-radius: 8px;
    ...
    /* Table Styles */
    .mirsaige-supplier-table {
        width: 100%;
        border-collapse: collapse;
    }

    .mirsaige-supplier-table thead {
        background: var(--mirsaige-darker-blue);
    }

    .mirsaige-supplier-table th {
        color: var(--mirsaige-accent);
    ...
    .mirsaige-supplier-table tr:last-child td {
        border-bottom: none;
    }

    .mirsaige-supplier-table tr:hover td {
        background: rgba(255, 178, 62, 0.05);
        color: var(--mirsaige-white);
    }

    /* Action Buttons */
    .mirsaige-supplier-actions {
        display: flex;
        gap: var(--mirsaige-space-xs);
        flex-wrap: nowrap;
    }

    .mirsaige-supplier-action-btn {
        padding: var(--mirsaige-space-3xs) var(--mirsaige-space-xs);
        border-radius: 4px;
        font-size: 0.8rem;
        font-weight: 500;
    ...
    .mirsaige-supplier-action-btn.view {
        background: var(--mirsaige-primary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-supplier-action-btn.edit {
        background: var(--mirsaige-secondary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-supplier-action-btn.delete {
        background:rgb(220, 51, 68);
        color:rgb(137, 3, 3);
    }

    .mirsaige-supplier-action-btn:hover {
        opacity: 0.9;
        transform: translateY(-1px);
    }

    /* Action text visibility - replaces JavaScript functionality */
    .mirsaige-supplier-action-btn .action-text {
        display: inline;
        color: var(--mirsaige-white);
    }
    ...
    /* Scrollbar Styling */
    .mirsaige-supplier-table-wrapper::-webkit-scrollbar {
        height: 8px;
    }

    .mirsaige-supplier-table-wrapper::-webkit-scrollbar-track {
        background: var(--mirsaige-dark-blue);
        border-radius: 4px;
    }

    .mirsaige-supplier-table-wrapper::-webkit-scrollbar-thumb {
        background: var(--mirsaige-accent);
        border-radius: 4px;
    }

    .mirsaige-supplier-table-wrapper::-webkit-scrollbar-thumb:hover {
    ...
    @media (max-width: 767px) {
        ...
        .mirsaige-supplier-table tbody {
            display: block;
            width: 100%;
        }
        
        .mirsaige-supplier-table tr {
            display: block;
            margin-bottom: var(--mirsaige-space-md);
        ...
        .mirsaige-supplier-table td:last-child {
            border-bottom: none;
        }
        
        .mirsaige-supplier-table td::before {
...
<div class="mirsaige-supplier-container">
    <div class="mirsaige-supplier-header">
        <div>
            <h1 class="mirsaige-app-breadcrumbs-title">Suppliers List</h1>
            <div class="mirsaige-app-breadcrumbs">
                ...
                <div class="mirsaige-app-breadcrumb">
                    <a href="{{ route('suppliers.index') }}" class="active">Manage Suppliers</a>
                </div>
                @endif
            </div>
        </div>
        ...
        <a href="{{ route('suppliers.create') }}" class="mirsaige-app-breadcrumbs-btn">
            <i class="fa-solid fa-square-plus"></i> <span class="action-text">Create</span>
        </a>
        @endif
    </div>

    <div class="mirsaige-supplier-table-wrapper">
        <div class="mirsaige-supplier-table-container">
            <table class="mirsaige-supplier-table">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if(in_array($user_role_id, [1, 2]))
                        <td data-label="Created By">{{ $supplier->creator->name ?? 'Unknown' }}</td>
                        @endif
                        <td data-label="Actions">
                            <div class="mirsaige-supplier-actions">
                                @if (in_array($user_role_id, [1, 2, 3]))
                                <a href="{{ route('suppliers.show', $supplier->id) }}" class="mirsaige-supplier-action-btn view">
                                    <i class="fa-solid fa-eye"></i> <span class="action-text">View</span>
                                </a>
                                <a href="{{ route('suppliers.edit', $supplier->id) }}" class="mirsaige-supplier-action-btn edit">
                                ...
                                <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="post" style="display: inline;">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="mirsaige-supplier-action-btn delete">
...
<script>
    ...
    const deleteButtons = document.querySelectorAll('.mirsaige-supplier-action-btn.delete');
