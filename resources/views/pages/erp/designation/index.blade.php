...
<style>
    /* Base Styles */
    .mirsaige-designation-container {
    ...
    /* Table Container */
    .mirsaige-designation-table-wrapper {
        width: 100%;
        overflow-x: auto;
    ...
    .mirsaige-designation-table-container {
        background: var(--mirsaige-dark-blue);
        border-radius: 8px;
    ...
    /* Table Styles */
    .mirsaige-designation-table {
        width: 100%;
        border-collapse: collapse;
    }

    .mirsaige-designation-table thead {
        background: var(--mirsaige-darker-blue);
    }

    .mirsaige-designation-table th {
        color: var(--mirsaige-accent);
    ...
    .mirsaige-designation-table tr:last-child td {
        border-bottom: none;
    }

    .mirsaige-designation-table tr:hover td {
        background: rgba(255, 178, 62, 0.05);
        color: var(--mirsaige-white);
    }

    /* Action Buttons */
    .mirsaige-designation-actions {
        display: flex;
        gap: var(--mirsaige-space-xs);
        flex-wrap: nowrap;
    ...
    .mirsaige-designation-action-btn {
        padding: var(--mirsaige-space-3xs) var(--mirsaige-space-xs);
        border-radius: 4px;
        font-size: 0.8rem;
        font-weight: 500;
    ...
    .mirsaige-designation-action-btn.view {
        background: var(--mirsaige-primary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-designation-action-btn.edit {
        background: var(--mirsaige-secondary);
        color: var(--mirsaige-accent);
    }

    .mirsaige-designation-action-btn.delete {
        background:rgb(220, 51, 68);
        color:rgb(137, 3, 3);
    }

    .mirsaige-designation-action-btn:hover {
        opacity: 0.9;
        transform: translateY(-1px);
    }

    /* Action text visibility - replaces JavaScript functionality */
    .mirsaige-designation-action-btn .action-text {
        display: inline;
        color: var(--mirsaige-white);
    ...
    /* Scrollbar Styling */
    .mirsaige-designation-table-wrapper::-webkit-scrollbar {
        height: 8px;
    }

    .mirsaige-designation-table-wrapper::-webkit-scrollbar-track {
        background: var(--mirsaige-dark-blue);
        border-radius: 4px;
    }

    .mirsaige-designation-table-wrapper::-webkit-scrollbar-thumb {
        background: var(--mirsaige-accent);
        border-radius: 4px;
    }

    .mirsaige-designation-table-wrapper::-webkit-scrollbar-thumb:hover {
    ...
    @media (max-width: 767px) {
        ...
        .mirsaige-designation-table tbody {
            display: block;
            width: 100%;
        }
        
        .mirsaige-designation-table tr {
            display: block;
            margin-bottom: var(--mirsaige-space-md);
        ...
        .mirsaige-designation-table td:last-child {
            border-bottom: none;
        }
        
        .mirsaige-designation-table td::before {
...
@section('page')
<div class="mirsaige-designation-container">
    <div class="mirsaige-designation-header">
        <div>
            <h1 class="mirsaige-app-breadcrumbs-title"> Designation List</h1>
            <div class="mirsaige-app-breadcrumbs">
                ...
                <div class="mirsaige-app-breadcrumb">
                    <a href="{{ route('designations.index') }}" class="active">Manage Designation</a>
                </div>
                @endif
            </div>
        </div>
        ...
        <a href="{{ route('designations.create') }}" class="mirsaige-app-breadcrumbs-btn">
            <i class="fa-solid fa-square-plus"></i> <span class="action-text">Create</span>
        </a>
        @endif
    </div>

    <div class="mirsaige-designation-table-wrapper">
        <div class="mirsaige-designation-table-container">
            <table class="mirsaige-designation-table">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td data-label="ID">{{ $designation->id }}</td>
                        <td data-label="Name">{{ $designation->name }}</td>
                        <td data-label="Description">{{ $designation->description }}</td>
                        @if(in_array($user_role_id, [1, 2]))
                        <td data-label="Created By">{{ $designation->creator->name ?? 'Unknown' }}</td>
                        <td data-label="Updated By">{{ $designation->updater->name ?? 'Unknown' }}</td>
                        @endif
                        <td data-label="Actions">
                            <div class="mirsaige-designation-actions">
                                @if (in_array($user_role_id, [1, 2,3]))
                                <a href="{{ route('designations.show', $designation->id) }}" class="mirsaige-designation-action-btn view">
                                    <i class="fa-solid fa-eye"></i> <span class="action-text">View</span>
                                </a>
                                <a href="{{ route('designations.edit', $designation->id) }}" class="mirsaige-designation-action-btn edit">
                                ...
                                <form action="{{ route('designations.destroy', $designation->id) }}" method="post" style="display: inline;">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="mirsaige-designation-action-btn delete">
...
<script>
    ...
    const deleteButtons = document.querySelectorAll('.mirsaige-designation-action-btn.delete');
